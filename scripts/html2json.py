...
def get_instruction_description(root):
    ...
    for entry in description:
        if entry.tag == "p":
            ...
            text_to_add = arrange_text(0, 80, text_to_add.strip())
            text += text_to_add + "\n\n"
        elif entry.tag == "ul":
            li = entry.xpath("li")
            for item in li:
                text_to_add = re.sub(r'[\n\t]+', " ", item.text_content())
                text_to_add = re.sub(r'[ ]{2,}', " ", text_to_add)
                text_to_add = arrange_text(2, 80, text_to_add.strip())
                text_to_add = "* " + text_to_add[2:]
                text += text_to_add + "\n"
    return text.strip()


def get_aliases(root, file_name):
...
def get_instruction(file_name):
    if file_name.endswith(".html"):
        ...
        file_handler = open(os.path.join(xhtml_path, file_name), "r")
        ...
        description = get_instruction_description(root)
        # Get the instruction aliases
        aliases = get_aliases(root, file_name)
        ...
        if len(class_headings) > 0:
            ...
            for ch in class_headings:
                ...
                fields = get_fields(regdiagram[0], file_name)
                eq_mask, uneq_masks = get_masks(regdiagram[0])
                encodings = get_encodings(regdiagram[0])
...
xhtml_path = os.path.realpath(sys.argv[1])
...
encodings = []
# Loop html files
for filename in os.listdir(xhtml_path):
    instruction = get_instruction(os.path.join(xhtml_path, filename))
    if instruction is not None:
        print "Processing %s..." % filename
        encodings.append(instruction)
# Try to save the result to JSON
xhtml_path = os.path.abspath(xhtml_path)
if len(encodings) > 0:
    json_path = os.path.realpath(sys.argv[2])
    ...
    try:
        ...
        print "Can't create %s" % json_path
else:
